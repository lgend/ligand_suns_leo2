...
import Chemistry (ParseS, evalParseS, pMotif2)
...
numAtoms :: V.Vector (V.Vector (VS.Vector Int)) -> Int
numAtoms = V.sum . fmap (V.sum . fmap VS.length)

greediestSolution :: [V.Vector (V.Vector (VS.Vector Int))] -> V.Vector (V.Vector (VS.Vector Int))
greediestSolution = maximumBy (comparing numAtoms)

--lenToTake :: [V.Vector (V.Vector (VS.Vector Int))] -> Int
--lenToTake = round (length / 10)
...
motifsInStructure (MotifGraphs motifs) structure
  = let 
        motifsParser =
            fmap V.fromList
          . mapM (fmap V.fromList . many . msum . map pMotif2)
          $ motifs
     ...
     in greediestSolution $ take 100 $ evalParseS motifsParser structure
